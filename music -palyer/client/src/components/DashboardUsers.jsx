...
export const DashboardUserCard = ({ data, index }) => {
  ...
  return (
    <motion.div
      key={index}
      className="relative w-full rounded-md flex items-center justify-between gap-2 p-4 bg-lightOverlay cursor-pointer hover:bg-card hover:shadow-md"
    >
      {/* Delete Button (Only if current user is admin and not deleting self) */}
      {user?.user?.role === "admin" && data._id !== user?.user._id && (
        <motion.div
          whileTap={{ scale: 0.75 }}
          className="right-4 w-8 h-8 rounded-md flex items-center justify-center bg-gray-200"
      ...
      {/* User image */}
      <div className="w-275 min-w-[160px] flex items-center justify-between">
        <img
          src={data.imageUrl}
      ...
      {/* Role & Toggle */}
      <div className="w-275 min-w-[160px] flex items-center justify-center gap-6 relative">
        <p className="text-base text-textColor text-center">{data.role}</p>

        {/* Toggle Role Button (Only for admin and not self) */}
        {user?.user?.role === "admin" && data._id !== user?.user._id && (
          <motion.p
            whileTap={{ scale: 0.75 }}
            className="text-[10px] font-semibold text-textColor px-1 bg-purple-200 rounded hover:shadow-md"
        ...
        {IsUserRoleUpdated && (
          ...
          >
            <p className="text-textColor text-[15px] font-semibold">
            ...
            <div className="flex items-center gap-4">
              <motion.button
                whileTap={{ scale: 0.75 }}
                className="text-sm px-4 py-1 rounded-md bg-blue-200 text-black hover:shadow-md"
                onClick={() =>
              ...
              <motion.button
                whileTap={{ scale: 0.75 }}
                className="text-sm px-4 py-1 rounded-md bg-gray-200 text-black hover:shadow-md"
...
const DashboardUsers = () => {
  ...
  // Show only the current user if not admin
  const visibleUsers =
    user?.user?.role === "admin"
      ? allUsers
      : allUsers?.filter((u) => u._id === user?.user?._id);
      ...
      <div className="relative w-full py-12 min-h-[400px] overflow-x-scroll my-4 flex flex-col items-center justify-start p-4 border border-gray-300 rounded-md gap-3">
        ...
        {visibleUsers &&
          visibleUsers.map((data, i) => (
            <DashboardUserCard key={data._id} data={data} index={i} />
          ))}
      </div>
    </div>
  );
